...
import com.chipsguide.app.colorbluetoothlamp.v2.brunton.activity.MainActivity;
import com.chipsguide.app.colorbluetoothlamp.v2.brunton.activity.TimeDeviceLightActivity;
...
public class BluetoothDeviceManagerProxy {
	...
	{
		if (bluzDeviceMan != null && deviceMusicManager != null
				&& localMusicManagerlistener != null
		...
		} else
		{
			...
			if (bluzDeviceMan == null)
			{
				localMusicManagerlistener
						.onMusicManagerReadyFailed(priorityMode);// 设置优先选择的模式
			} else
			{
				if (isMusicManagerMode(deviceManagerMode))
				{// 是否为音乐管理相关模式
	...
	public static class SimpleDeviceUiChangedListener implements
			OnDeviceUiChangedListener {
		@Override
		public void onVolumeChanged(boolean firstCallback, int volume,
				boolean on)
		{
		}
	}

	public static class SimpleDeviceConnectStateChangedListener implements
			OnDeviceConnectedStateChangedListener {
		@Override
	...
	/**
	 * 添加设备UI相关变化监听，如音量 注：退出界面时，调用removeDeviceUiChangedListener
	 * 
	 * @param listener
	 */
	...
	/**
	 * 添加设备连接变化 注：退出界面时，调用removeDeviceConnectedStateChangedListener
	 * 
	 * @param listener
	 */
	public void setDeviceConnectedStateChangedListener(
			OnDeviceConnectedStateChangedListener listener)
	{
	...
	/**
	 * 添加连接状态改变监听 如果添加的类为Activity、Fragment或其中的非静态内部类，<br>
	...
	private OnBluetoothDeviceConnectionStateChangedListener connStateChangeListener = new OnBluetoothDeviceConnectionStateChangedListener()
	{
				...
				BluetoothDevice device, int state)
		{
			...
			{
				// 连接状态的改变
				mDeviceConnectedStateChangedListener
	...
	private OnBluetoothDeviceGlobalUIChangedListener globalUiChangedListener = new OnBluetoothDeviceGlobalUIChangedListener()
	{
		...
		public void onBluetoothDeviceModeChanged(int mode)
		{
							...
							.setOnBluetoothDeviceAlarmManagerReadyListener(new OnBluetoothDeviceAlarmManagerReadyListener()
							{
								...
								public void onBluetoothDeviceAlarmManagerReady()
								{
									...
									if (!(CustomApplication.getActivity() instanceof TimeDeviceLightActivity)
											&& CustomApplication.isClickAlarm)
									{
										Intent intent = new Intent(
												CustomApplication.getActivity(),
												TimeDeviceLightActivity.class);
										CustomApplication.getActivity()
			...
			notifyModeChangedListener(mode);
		...
		public void onBluetoothDeviceVolumeChanged(int volume, boolean on)
		{
			...
			if (mDeviceUiChangedListener != null)
			{
				mDeviceUiChangedListener.onVolumeChanged(volumeFirstCallback,
						volume, on);
			}
		}
	};
	...
	private OnBluetoothDeviceCardMusicManagerReadyListener cardMusicReadyListener = new OnBluetoothDeviceCardMusicManagerReadyListener()
	{
		...
		public void onBluetoothDeviceCardMusicManagerReady()
		{
			...
			{
				localMusicManagerlistener
						.onMusicManagerReadyFailed(deviceManagerMode);
			} else
			{
				localMusicManagerlistener.onMusicManagerReady(
						deviceMusicManager, deviceManagerMode);
	...
	public void destory()
	{
		...
		if (bluzDeviceMan != null)
		{
			...
			bluzDeviceMan
					.setOnBluetoothDeviceConnectionStateChangedListener(null);
	...
	private void notifyModeChangedListener(int mode)
	{
		...
		for (int i = 0; i < size; i++)
		{
			modeList.get(i).onModeChanged(mode);
		}
	}

	private List<OnModeChangedListener> modeList = new ArrayList<OnModeChangedListener>();

	public void addOnModeChangedListener(OnModeChangedListener listener)
	{
		modeList.add(listener);
	}

	// 销毁时掉用
	public void removeOnModeChangedListener(
			OnModeChangedListener listener)
	{
		modeList.remove(listener);
	}
